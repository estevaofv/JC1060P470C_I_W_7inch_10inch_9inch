...
/* JC1060P470C: speaker amplifier enable is on GPIO20 (devkit uses GPIO53) */
...
/* JC1060P470C: backlight is on GPIO23 (devkit uses GPIO26) */
#define BSP_LCD_BACKLIGHT     (GPIO_NUM_23)
#define BSP_LCD_RST           (GPIO_NUM_27)
/* GT911 has its own reset/int lines here (not shared with the panel reset).
 * They must be driven: the GT911 latches its I2C address (0x5D) from the INT
 * level while RST is released. Left as NC the controller never answers and
 * esp_lcd_touch_new_i2c_gt911() fails with "i2c transaction failed".
