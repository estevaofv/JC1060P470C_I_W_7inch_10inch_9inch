...
#if CONFIG_BSP_LCD_TYPE_1024_600
/*
 * JC1060P470C board deltas (see 1-Demo/Demo_IDF/examples/readme/readme.txt):
 * the JD9165 panel fitted to this board needs its own vendor init sequence,
 * which is passed through jd9165_vendor_config_t::init_cmds so the stock
 * esp_lcd_jd9165 component stays untouched and updatable.
 */
...
static void bsp_touch_scale_coordinates(esp_lcd_touch_handle_t tp, uint16_t *x, uint16_t *y,
                                        uint16_t *strength, uint8_t *point_num, uint8_t max_point_num)
{
    ...
    for (uint8_t i = 0; i < *point_num; i++) {
        x[i] = (uint16_t)(((uint32_t)x[i] * BSP_LCD_H_RES) / BSP_TOUCH_NATIVE_H_RES);
        y[i] = (uint16_t)(((uint32_t)y[i] * BSP_LCD_V_RES) / BSP_TOUCH_NATIVE_V_RES);
    }
}
...
esp_err_t bsp_display_new_with_handles(const bsp_display_config_t *config, bsp_lcd_handles_t *ret_handles)
{
...
#if CONFIG_BSP_LCD_TYPE_1024_600
    // create JD9165 control panel (JC1060P470C panel, replaces the devkit's EK79007)
    ...
    /* Timings validated on JC1060P470C hardware; they differ from the stock
     * esp_lcd_jd9165 macro (50MHz, hpw 20, vbp 12, vfp 20). */
...
esp_err_t bsp_touch_new(const bsp_touch_config_t *config, esp_lcd_touch_handle_t *ret_touch)
{
    ...
    const esp_lcd_touch_config_t tp_cfg = {
        ...
        .flags = {
            /* JC1060P470C panel is mounted straight — no swap/mirror */
            .swap_xy = 0,
            .mirror_x = 0,
            .mirror_y = 0,
        },
#if CONFIG_BSP_LCD_TYPE_1024_600
        .process_coordinates = bsp_touch_scale_coordinates,
#endif
    };
